...
`yarn test`). Do not proceed to "Ready for review" until it passes.
...
> **repo-specific (wazuh-dashboard-reporting):** the changelog is
> [`CHANGELOG.md`](../../../CHANGELOG.md) at the repo root, grouped under a
> `## [v<x>]` heading with `### Added` / `### Changed` / `### Fixed` / `### Removed`
> subsections. Group with an existing entry if the PR continues a previously merged
> feature. (Many historical entries link to PRs; for new work, prefer the **issue**
> link.)
...
Fill the template file (or a working copy of it) with real content before
passing it via `--body-file`; do not inline the body as a heredoc.
