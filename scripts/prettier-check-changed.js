...
try {
  ...
  console.error(
    'Prettier is not installed. Run the dependency install step before this check.'
  );
  process.exit(2);
}
...
function main() {
  ...
  if (!staged && bases.length === 0) {
    console.error(
      'usage: prettier-check-changed.js (--base <ref> | --staged) [--fix] <file>...'
    );
    process.exit(2);
  }
  if (files.length === 0) {
  ...
  for (const ref of bases) {
    if (
      gitQuiet('rev-parse', '--verify', '--quiet', `${ref}^{commit}`) === null
    ) {
      console.error(
  ...
  const renamesFor = (ref) => {
    ...
    const ns =
      gitQuiet(
        ...(ref === '--cached' ? [...args, '--cached'] : [...args, ref])
      ) || '';
  ...
  for (const ref of staged ? ['--cached'] : mergeBases)
    renameMaps.set(ref, renamesFor(ref));
  ...
  for (const raw of files) {
    try {
      ...
      for (const ref of refs) {
        ...
        const existed =
          staged || gitQuiet('cat-file', '-e', `${ref}:${src}`) !== null;
        let d = null;
        if (existed && staged) {
          d = gitQuiet(
            'diff',
      ...
      const tmp = path.join(
        fs.mkdtempSync(path.join(os.tmpdir(), 'pcc-')),
        path.basename(file)
      );
      ...
      if (fix) {
        ...
        const keep = all.filter(
          (h) => h.oc > 0 && overlaps([[h.os, h.os + h.oc - 1]], written)
        );
        ...
        console.log(
          `fixed  ${file} (${keep.length} hunk${keep.length === 1 ? '' : 's'})`
        );
      } else {
  ...
  console.log(
    'These lines were written by this change and are not formatted:\n'
  );
  for (const { file, hits } of problems) {
    for (const [s, e] of hits)
      console.log(`  ${file}:${s}${e > s ? `-${e}` : ''}`);
  }
  const self =
    path.relative(root, process.argv[1]) || path.basename(process.argv[1]);
